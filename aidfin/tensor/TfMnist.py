...
from aidfin.conf.GlobalSettings import *
"""
...
tf.app.flags.DEFINE_string('data_dir', TRAIN_DATA,
                           ...
                           """and checkpoint.""")
tf.app.flags.DEFINE_string('log_dir', EVENT_LOGS,
                           """Directory where to write event logs """
                           """and checkpoint.""")
tf.app.flags.DEFINE_integer('max_steps', MAX_STEPS,
                            """Number of batches to run.""")
tf.app.flags.DEFINE_boolean('log_device_placement', LOG_DEVICE_PLACEMENT,
                            """Whether to log device placement.""")
tf.app.flags.DEFINE_float('learning_rate',LEARNING_RATE,"""learning rate""")
tf.app.flags.DEFINE_float('dropout',DROPOUT,"""dropout""")
...
# 定义神经网络模型参数的初始化方法，
# 权重依然使用常用的truncated_normal进行初始化，符合正态分布的数据数值，偏置则赋值为0.1
...
for i in range(FLAGS.max_steps):
    if i % 10 == 0:
        ...
        if i % 100 == 99:
            ...
            saver.save(sess, FLAGS.log_dir + "/model.ckpt", i)
